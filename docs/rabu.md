...
| position                             | position_lat, position_lon, position_alt | Input - Integer, Integer, Float | | Current GNSS coordinates of the vehicle. Latitude and longitude are specificied in WGS84 and multiplied by 10e7. Altitude is meters above average sea level (ASL). |
| set_position                         | setposition_lat, setposition_lon, setposition_alt | Input - Integer, Integer, Float | | Sends to the autopilot the desidered GNSS coordinates of the vehicle. Latitude and longitude are specificied in WGS84 and multiplied by 10e7. Altitude is meters above average sea level (ASL). |